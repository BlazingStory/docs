...
- `Docs/` — the Blazor WASM app (`BlazingStory.Docs.csproj`).
  ...
  - `Services/`
    ...
    - `DocRoutes` — the single source of truth for every URL/path shape (version catalog, sidebar file, markdown file, content/asset file, doc link) and for parsing a raw URL path into `(version, slug)` via `ResolveDocPath`, used by both `DocPage` and `DocumentPreloadService`. Route logic changes belong here. Every URL it builds for a real file also carries a `?v={commit}` cache buster, read once from this assembly's `AssemblyInformationalVersion`, where the .NET SDK writes the Git commit of the build on its own. That is what keeps a published build from reading an older build's `.md`, `.json`, or `.vec` out of the browser cache. Append it inside `DocRoutes` and nowhere else, because `DocPage`'s `<link rel="preload">` hints and the `HttpClient` fetches have to end up with the very same URL, or every file is downloaded twice.
