...
- Everything the browser loads that is not committed is built by a target of `BlazingStory.Docs.csproj`, and all of them hang off `Restore` and `Build` so that the Static Web Assets manifest of the same build already has what they write (see Prerendering below for why that matters). `CompileTypeScript` depends on the two bundle targets, since `tsc` cannot resolve the transformers.js import until `TransformersBundle.targets` has written its `.d.ts`. Order aside, each one skips itself when its outputs are newer than its inputs, so a build that changes nothing costs about a second.
